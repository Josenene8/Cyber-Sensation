...
class CustomControlsState extends MusicBeatSubstate
{
	...
	public static var peniscmamut:Bool = false;
	// 'hitbox',
	var controlitems:Array<String> = ['right control', 'left control', 'keyboard', 'custom', 'hitbox', '5k pad'];
	...
	public function new()
	{
		...
		// pad
		_pad = new FlxVirtualPad(FULL, ELPEPE);
		_pad.alpha = 0;
	...
	override function update(elapsed:Float)
	{
		...
		for (touch in FlxG.touches.list)
		{
			...
			// change Selection
			if (touch.overlaps(leftArrow) && touch.justPressed || FlxG.mouse.overlaps(leftArrow) && FlxG.mouse.justPressed)
			{
				changeSelection(-1);
			}
			else if (touch.overlaps(rightArrow) && touch.justPressed || FlxG.mouse.overlaps(rightArrow) && FlxG.mouse.justPressed)
			{
				changeSelection(1);
			}
	...
	function changeSelection(change:Int = 0, ?forceChange:Int)
	{
		...
		switch curSelected
		{
			case 0:
				this.remove(_pad);
				_pad = null;
				_pad = new FlxVirtualPad(RIGHT_FULL, ELPEPE);
				_pad.alpha = 0.75;
				this.add(_pad);
			case 1:
				this.remove(_pad);
			...
			if (_pad.buttonUp2.justPressed)
				{
					...
					movebutton(touch, _pad.buttonUp2);
				}
		}
	}
